---
layout: page
title: MathDeck · Terms of Use
permalink: /apps/mathdeck/terms/
---

**Effective date:** 2026-04-16
**Last updated:** 2026-04-16

These Terms of Use ("Terms") govern your access to and use of
the MathDeck Android application (the "app") published by
Omiverse Labs ("we", "us", "our"). By downloading, installing,
or using MathDeck you agree to these Terms.

---

## 1. License

We grant you a personal, non-exclusive, non-transferable,
revocable license to install and use MathDeck on devices you
own or control, for your own non-commercial or commercial use,
subject to these Terms and the Google Play
[Terms of Service](https://play.google.com/about/play-terms/).

You may not:

- Reverse-engineer, decompile, or disassemble the app, except
  to the extent such restriction is prohibited by applicable
  law.
- Remove or alter any proprietary notices.
- Redistribute, sublicense, sell, rent, or lease the app or any
  part of it.
- Use the app to build a competing product.

---

## 2. No Professional Advice

MathDeck provides calculators for general reference and
educational purposes. The financial, engineering, health, and
other domain-specific calculators (EMI, interest, Ohm's law,
BMI, beam bending, etc.) are **not** a substitute for advice
from a qualified professional.

Do not rely on MathDeck alone for:

- Medical, nutritional, or fitness decisions
- Financial, tax, investment, or loan decisions
- Structural, electrical, or other engineering decisions
- Legal or regulatory compliance decisions

Always consult a licensed professional where expert judgment is
required. You are responsible for verifying any critical result
independently.

---

## 3. Currency Exchange Rates

The Currency Converter fetches public exchange rates from a
third-party service (`open.er-api.com`). Rates are provided
for reference only, are not continuous, and may differ from
the rate your bank or payment processor will apply to any
real transaction. We do not guarantee the availability,
accuracy, or timeliness of this data.

---

## 4. No Warranty

THE APP IS PROVIDED "AS IS" AND "AS AVAILABLE", WITHOUT
WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT
LIMITED TO MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE,
OR NON-INFRINGEMENT. WE DO NOT WARRANT THAT THE APP WILL BE
UNINTERRUPTED, ERROR-FREE, OR FREE FROM DEFECTS, OR THAT
CALCULATION RESULTS WILL BE ACCURATE FOR YOUR SPECIFIC USE
CASE.

---

## 5. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, OMIVERSE
LABS SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL,
CONSEQUENTIAL, OR PUNITIVE DAMAGES, OR ANY LOSS OF PROFITS,
REVENUES, DATA, OR GOODWILL, ARISING FROM OR RELATED TO YOUR
USE OF, OR INABILITY TO USE, THE APP. IN NO EVENT SHALL OUR
TOTAL AGGREGATE LIABILITY EXCEED THE AMOUNT YOU PAID FOR THE
APP IN THE 12 MONTHS PRECEDING THE CLAIM (WHICH, FOR A FREE
APP, IS ZERO).

Some jurisdictions do not allow the exclusion or limitation of
certain warranties or liabilities; in those jurisdictions our
liability is limited to the greatest extent permitted by law.

---

## 6. Privacy

Your use of MathDeck is also governed by our
[Privacy Policy](../privacy/). Because MathDeck does not
collect personal data, there is no server-side account or
profile tied to you.

---

## 7. Changes to the App or Terms

We may update the app or these Terms at any time. Material
updates to the Terms will be reflected in the "Last updated"
date at the top of this page and, where required, through
notice in the app or on this website. Continued use after the
effective date of a change constitutes acceptance of the
revised Terms.

---

## 8. Termination

You may stop using MathDeck at any time by uninstalling it. We
may suspend or terminate your access to the app if you breach
these Terms or use the app in a manner that could cause harm
to us or third parties.

Sections 2-5, 9, and 10 survive termination.

---

## 9. Governing Law

These Terms are governed by the laws of the jurisdiction in
which Omiverse Labs is established, without regard to its
conflict-of-laws rules. Disputes shall be resolved in the
courts of that jurisdiction, except where applicable consumer
protection law grants you a non-waivable right to bring a
claim in your local courts.

---

## 10. Contact

Questions about these Terms: [support@omiverselabs.com](mailto:support@omiverselabs.com)
