---
layout: page
title: TEMPLATE_APP_NAME · Terms of Use
permalink: /apps/TEMPLATE_APP_SLUG/terms/
---

**Effective date:** YYYY-MM-DD
**Last updated:** YYYY-MM-DD

These Terms of Use ("Terms") govern your use of the
TEMPLATE_APP_NAME application (the "app") published by
Omiverse Labs. By downloading, installing, or using the app,
you agree to these Terms.

---

## 1. License

We grant you a personal, non-exclusive, non-transferable,
revocable license to install and use TEMPLATE_APP_NAME on
devices you own or control, subject to these Terms and the
applicable platform store terms (Google Play, App Store, etc.).

You may not reverse-engineer, redistribute, sublicense, sell,
rent, or lease the app, except where applicable law expressly
overrides this restriction.

---

## 2. No Professional Advice

_Replace or keep depending on the app's purpose. For utility /
reference apps, keep a disclaimer telling users not to rely on
the app for medical, financial, legal, engineering, or
regulatory decisions without consulting a qualified
professional._

---

## 3. No Warranty

THE APP IS PROVIDED "AS IS" AND "AS AVAILABLE", WITHOUT
WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT
LIMITED TO MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE,
OR NON-INFRINGEMENT.

---

## 4. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, OMIVERSE
LABS SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL,
CONSEQUENTIAL, OR PUNITIVE DAMAGES, OR ANY LOSS OF PROFITS,
REVENUES, DATA, OR GOODWILL, ARISING FROM OR RELATED TO YOUR
USE OF THE APP. OUR TOTAL AGGREGATE LIABILITY SHALL NOT EXCEED
THE AMOUNT YOU PAID FOR THE APP IN THE 12 MONTHS PRECEDING
THE CLAIM.

Some jurisdictions do not permit these limitations; in those
jurisdictions our liability is limited to the maximum extent
permitted.

---

## 5. Privacy

Your use of TEMPLATE_APP_NAME is also governed by our
[Privacy Policy](../privacy/).

---

## 6. Changes to the App or Terms

We may update the app or these Terms at any time. Material
updates will be reflected in the "Last updated" date above.
Continued use after the effective date of a change constitutes
acceptance.

---

## 7. Termination

You may stop using the app at any time by uninstalling it. We
may suspend or terminate your access if you breach these Terms.

Sections 2-4, 8, and 9 survive termination.

---

## 8. Governing Law

These Terms are governed by the laws of the jurisdiction in
which Omiverse Labs is established, without regard to its
conflict-of-laws rules.

---

## 9. Contact

Questions about these Terms: [support@omiverselabs.com](mailto:support@omiverselabs.com)
